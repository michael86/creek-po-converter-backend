...
const queries: Queries = {
  parts: {
    ...
    insertPrefix: async (prefix: string) => {
      try {
        const res = await runQuery<PutRequest>(`insert into prefixes (prefix) values (?)`, [
          prefix,
        ]);
  ...
  purchaseOrders: {
      ...
      try {
        ...
        for (const part of data.DATA) {
          const [sku, quantity] = await Promise.all([
            runQuery(`insert into part_number (part, description) values (?, ?)`, [
              part[0],
              part[2],
            ]),
    ...
    patchPartialStatus: async (order: string, name: string) => {
      ...
      try {
        const id = await runQuery(`SELECT id from purchase_order WHERE purchase_order = ?`, [
          order,
        ]);
    ...
    addParcelsToOrder: async (parcels: number[], purchaseOrder: string, part: string) => {
      try {
        ...
        const purchaseId = await runQuery(
  ...
  user: {
    ...
    validateLogin: async (email: string) => {
      try {
        const user = await runQuery<FecthRequest>(
    ...
    getUserRole: async (email: string) => {
      try {
        const role = await runQuery<FecthRequest>("select role from users where email =? ", [
          email,
        ]);
